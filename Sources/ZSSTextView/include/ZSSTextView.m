...
@interface ZSSTextView()

    @property (nonatomic, retain) NSObject* html_tags_color;
    @property (nonatomic, retain) NSObject* a_tag_color;
    @property (nonatomic, retain) NSObject* img_tag_color;
    @property (nonatomic, retain) NSObject* html_entities_color;
    @property (nonatomic, retain) NSObject* comments_color;
    @property (nonatomic, retain) NSObject* attributes_color;
...
- (void) commonSetup {
    ...
    _defaultFont = [UIFont systemFontOfSize: fontSize];
    _boldFont = [UIFont boldSystemFontOfSize: fontSize];
    _italicFont = [UIFont fontWithName: @"HelveticaNeue-Oblique" size: fontSize];

    _html_tags_color = [UIColor systemBlueColor];  // RGB(25, 0, 151)
    _a_tag_color = [UIColor systemGreenColor];     // RGB(24, 104, 10)
    _img_tag_color = [UIColor systemPurpleColor];  // RGB(148, 0, 151)
    _html_entities_color = [UIColor labelColor];   // this works for &nbsp; etc.
    _comments_color = [UIColor systemGreenColor];  // RGB(31, 131, 0)
    _attributes_color = [UIColor systemBlueColor]; // RGB(0, 12, 255)
    ...
    if (_italicFont == nil && ([UIFontDescriptor class] != nil)) {
        ...
        _italicFont = (__bridge_transfer UIFont*)CTFontCreateWithName(CFSTR("HelveticaNeue-Italic"), fontSize, NULL);
    }
...
- (NSArray*) solverTokens {
    ...
    NSArray *solverTokens = @[
        [CYRToken tokenWithName: @"html_tags"
                     expression: @"(<\\/?[a-z][^<>]*>)"
                     attributes: @{
                                 NSForegroundColorAttributeName : _html_tags_color}],
        [CYRToken tokenWithName: @"a_tag"
                     expression: @"(<\\/?[a][^<>]*>)"
                     attributes: @{
                                 NSForegroundColorAttributeName : _a_tag_color}],
        [CYRToken tokenWithName: @"img_tag"
                     expression: @"(<\\/?[img][^<>]*>)"
                     attributes: @{
                                 NSForegroundColorAttributeName : _img_tag_color}],
